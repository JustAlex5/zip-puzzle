...
export class LevelService {
  ...
  /** POST /levels — creates a level; backend validates solvability. */
  createLevel(dto: {
    name: string;
    size: number;
  ...
  /** POST /levels/{id}/solve — increments solve count on the server. */
  incrementSolveCount(id: number): Observable<LevelDto> {
    return this.http
      .post<ApiResponse<LevelDto>>(`${environment.apiUrl}/levels/${id}/solve`, {})
      .pipe(
        map((res) => {
          const data = res.data;
          if (data == null) {
